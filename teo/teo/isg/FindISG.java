...
public class FindISG {
        ...
        for (int i = 0; i < configurations.size(); i++)
            for (int j = 0; j < graphs.size(); j++) {
                ...
                if (!g.getName().startsWith("USG") && C.isInducedSubgraph(g)) {
                     ...
                     * " ist in allen Repräsentanten von " + C.getName() +
                     * " enthalten \n");
                     */
                    C.addInduced(g);
                }
    ...
    private static void usage() {
        System.err.println("Usage: FindISG [opts] input.xml out.xml\n"
    ...
    public static void makeNewXMLFile(String outFile) throws IOException,
            SAXException {
    ...
    /**
     * Create a list of direct subgraphs for each graph and check to which graph
     * these subgraphs are isomorph. If no isomorph graphs are found, add the
     * subgraph to the list of graphs.
     */
    ...
    private static void readXMLFile(String file) throws Exception {
        ...
        XMLParser xr = new XMLParser(input, handler, null, new NoteFilter(
                SmallGraphTags.EXPL));
        xr.parse();
        ...
        for (i = 0; i < graphs.size(); i++)
            for (j = i + 1; j < graphs.size(); j++)
                if (((Graph) graphs.elementAt(i)).isIsomorphic((Graph) graphs
                        .elementAt(j)))
            ...
            for (j = i + 1; j < configurations.size(); j++)
                if (((Configuration) configurations.elementAt(i))
                        .isIsomorphic((Configuration) configurations
                                .elementAt(j)))
                    System.err.println("Mistake!!! "
                            + ((Configuration) configurations.elementAt(i))
                                    .getName()
                            + " isomorphic to "
                            + ((Configuration) configurations.elementAt(j))
                                    .getName());
                    ...
                    contFHMT: for (j = 0; j < smMem.size(); j++)
                        if (((Graph) smMem.elementAt(j)).countNodes() <= maxCnt) {
                            for (int k = 0; k < graphs.size(); k++)
                                if (((Graph) smMem.elementAt(j))
                                        .isIsomorphic((Graph) graphs
                                                .elementAt(k))) {
                                    smMem.setElementAt(
                                            (Graph) graphs.elementAt(k), j);
                                    continue contFHMT;
                                }
                            ((Graph) smMem.elementAt(j)).addLink(fhmt
                                    .getLink());
        ...
        for (i = 0; i < families.size(); i++)
            if ((families.elementAt(i) instanceof HMTFamily)
                ...
                /*
                 * Graph[] compSmMem = new Graph[smMem.size()]; for (j=0;
                 * j<smMem.length; j++) if (smMem[j].countNodes()<=maxCnt)
    ...
    private static void sortNum(Vector vec, int left, int right) {
        ...
        while (i <= j) {
            while (n2n(vec.elementAt(i)) < x)
                i++;
            while (n2n(vec.elementAt(j)) > x)
                j--;
            if (i <= j) {
    ...
    public static void addBigSmallmembers() throws IOException,
            InterruptedException {
                    ...
                    contBig: for (int j = 0; j < smMem.size(); j++)
                        if (((Graph) smMem.elementAt(j)).countNodes() > maxCnt) {
                            for (int k = 0; k < bigSmallmemb.size(); k++)
                                if (((Graph) smMem.elementAt(j)).countNodes() == ((Graph) bigSmallmemb
                                        .elementAt(k)).countNodes()
                                        && isSubgraphVF(
                                                (Graph) smMem.elementAt(j),
                                                (Graph) bigSmallmemb
                                                        .elementAt(k))) {
                                    smMem.setElementAt(
                                            ...
                                            j);
                                    compSmMem
                                            .addElement((Graph) ((Graph) bigSmallmemb
                                                    .elementAt(k))
                                                    .getComplement());
                                    continue contBig;
                                }
                            ((Graph) smMem.elementAt(j)).addLink(fhmt
                                    .getLink());
                            addUSG((Graph) smMem.elementAt(j), bigSmallmemb,
                                    ISG);
                             ...
                             * System.out
                             * .println(fcomp.getName()+".smMem["+j+"]="+
                             * ((Graph)compSmMem.elementAt(j)).getName());
                             */
                        } else
                            compSmMem.addElement((Graph) fcomp
                                    .getSmallmembers().elementAt(j));
        ...
        /*
         * for (int j=0; j<topo.size(); j++) for (int k=0; k<topo.size(); k++) {
         * if (j == k) continue; if
         * (resultGraph.getPath((Node)topo.elementAt(j),
         * (Node)topo.elementAt(k)).size() == 0) { if (isSubgraphVF( (Graph)
         * ((Node)topo.elementAt(j)).getData(grIndex), (Graph)
         * ((Node)topo.elementAt(k)).getData(grIndex))) {
         * System.out.println("VF found: "+ ((Graph)((Node)topo.elementAt(k)).
         * getData(grIndex)).getName()+" ind.s.g. of "+
         * ((Graph)((Node)topo.elementAt(j)). getData(grIndex)).getName()); if
         * (((Graph)((Node)topo.elementAt(k)).
         * getData(grIndex)).getName().startsWith(ISG))
         * System.out.println(((Graph)((Node)topo.elementAt(k)).
         * getData(grIndex)).toString()); if (((Graph)((Node)topo.elementAt(j)).
         * getData(grIndex)).getName().startsWith(ISG))
         * System.out.println(((Graph)((Node)topo.elementAt(j)).
         * getData(grIndex)).toString()); } } else if (!isSubgraphVF( (Graph)
         * ((Node)topo.elementAt(j)).getData(grIndex), (Graph)
         * ((Node)topo.elementAt(k)).getData(grIndex))) {
     ...
     */
    public static boolean isSubgraphVF(Graph large, Graph small)
