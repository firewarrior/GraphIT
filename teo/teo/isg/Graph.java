...
public class Graph extends SmallGraph {
    ...
    private void updateKomponentenVektor() {
        ...
        for (i = 0; i < cnt; i++) {
            for (j = 0; j < cnt; j++) {
                if (matrix[i][j]
                        && KomponentenVektor[i] != KomponentenVektor[j]) {
                    int x, y;
    ...
    /**
     * Returns a string that represents the graph. It contains the number of
     * nodes, the name(s) of the graph and the list of edges.
     */
     ...
     * mask[1] <tt>g<tt>.
     */
    private boolean isIsomorphicIntern(Graph g, boolean darf[][],
            boolean mask[][]) {
        int i, j;
        ...
        /*
         * Speichere in Grad[0][i] den Grad des Knoten i im Graph this und setze
         * KnotenMitGrad[x][0][y] auf true, wenn im Graphen this der Knoten y
         * den Grad x hat. Analog für den Graphen g wenn der Index 1 gewählt
         * ist. Das ganze aber nur dann, wenn in der jeweiligen Maske das Flag
         * für den entsprechenden Knoten gesetzt ist.
         */
        for (i = 0; i < cnt; i++) {
            int h;
    ...
    private boolean isIsomorphicIntern2(Graph g, boolean darf[][],
            boolean maskAlt[][], boolean maskNeu[][]) {
        int i, j;
        ...
        for (i = 0; i < cnt; i++)
            if (maskNeu[0][i])
                for (j = 0; j < cnt; j++)
                    if (maskNeu[1][j]
                            && SummeGradNachbarn[0][i] != SummeGradNachbarn[1][j]) {
                        darf[i][j] = false;
                    }
    ...
    public boolean isSubIsomorphic(Graph g) {
        ...
        /*
         * jetzt haben wir g als Graphen mit einer Knotenzahl g.m von 4 <= g.m <
         * this.m
         * 
    ...
    /**
     * Checks whether the matrices of <tt>this</tt> and <tt>g</tt> are equal if
     * the nodes of <tt>g</tt> are reordered according to the permutation given
     * by <tt>perm</tt>. If the result is true, <tt>perm</tt> contains a mapping
    ...
    public static void main(String args[]) {
        /*
         * Graph g1 = new Graph(5); Graph g2 = new Graph(5);
         * 
         * g1.addEdge(0,1); g1.addEdge(1,2); g1.addEdge(2,3); g1.addEdge(3,4);
         * g1.addEdge(0,3);
         * 
         * g2.addEdge(0,1); g2.addEdge(1,2); g2.addEdge(2,3); g2.addEdge(0,4);
         * g2.addEdge(2,4);
         */
