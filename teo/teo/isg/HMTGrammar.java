...
public class HMTGrammar {
    ...
    public class HMTGraph extends Graph {
        ...
        /**
         * Create a new graph with the given attachment/extension. Att/ext are
         * arrays containing the nodes in att/ext, in the proper order. That is,
         * if att(0) = 0, att(5) = 1, att(2) = 2, then att=[0,5,2]. If the graph
         * has no att (ext), use null.
         */
        public HMTGraph(Graph graph, int[] ext, int[] att) {
            super(graph);
            if (att != null && att.length != type || ext != null
                    && ext.length != type)
