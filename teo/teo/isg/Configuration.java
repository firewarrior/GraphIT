...
public class Configuration extends SmallGraph {
    ...
    /*
     * public SmallGraph makeComplement() { Configuration c = new
     * Configuration(this); c.complement(); setComplement(c);
     * c.setComplement(this); return c; }
     */
    ...
    /**
     * Adds an edge to the Configuration. <tt>type</tt> shows which edge should
     * be added.
     */
    ...
    /**
     * Returns the degree of type <tt>type</tt> of the node at index <tt>v</tt>.
     */
    ...
    /**
     * Returns the degree of type <tt>type</tt> of the node at index <tt>v</tt>
     * in the subgraph induced by <tt>mask<tt>.
     */
    ...
    public Vector<Graph> getGraphs(int maxGraphs) {
        ...
        if (DEBUG)
            System.out.print("  Kantenabbildungen: " + Transformationen.size()
                    + "\n");
        ...
        loop: for (i = 0; i < power2n; i++) {
            ...
            /*
             * ueberpruefen, ob neuer Repraesentant isomorph zu bereits in Liste
             * enthaltenen Repraesentanten ist. Der obige Test deckt nicht alle
             * Fälle ab.
             */
            ...
            if (confGraphs.size() > maxGraphs) {
                ...
                /*
                 * rep.setBottom(); System.err.print("Erzeuge Bottom-Graph\n");
                 * break;
                 */
     ...
     */
    private boolean isIsomorphicIntern(Configuration c, boolean darf[][],
            boolean mask[][]) {
        int i, j;
        ...
        for (i = 0; i < cnt; i++) {
            ...
            for (j = 0; j < cnt; j++) {
                ...
                if ((Grad[0][i] != Grad[1][j] || OptGrad[0][i] != OptGrad[1][j])
                        && darf[i][j]) {
                    changed = true;
                    darf[i][j] = false;
                }
    ...
    public boolean isInducedSubgraph(Graph g) {
        ...
        case 1:
            /*
             * eine optionale Kante -> beide Repräsentanten erzeugen und testen
             */
    ...
    /**
     * Checks whether the matrices of <tt>this</tt> and <tt>c</tt> are equal if
     * the nodes of <tt>c</tt> are reordered according to the permutation given
