...
public class HierarchyLayout<V, E> {
     ...
     * 
     * @param g
     *            the graph that will be layed out by calling layoutGraph()
     * @param vertexFactory
     *            is used for generating virtual nodes, the vertices it
     *            generates should all be not equal().
     * @param widthFunc
     *            should return the width for a vertex in g. It is called at the
     *            beginning of the layout algorithm. If widthFunc is null, a
     *            default width is used.
     */
    public HierarchyLayout(SimpleDirectedGraph<V, E> g,
     ...
     * ============================================================== structure
     * of function calls: feasibleTree() initRank() tightTree() findDeltaRank()
     * initCutValues() getCutValue() rank() enterEdge() exchange() getCutValue()
     * (see initCutValues) normalize() balance()
     */

    /**
     * Main procedure of the network simplex algorithm. This is the first pass
     * of laying out the graph for drawing. It's purpose is to assigns each node
     * of the graph to an integer rank.
     * 
    ...
    protected void rank(int pass) {
        ...
        /*
         * { // find maximum rank //GraphDrawInfo gdi; int max = 0; Enumeration
         * eenum = graph.getNodes(); Node node; while(eenum.hasMoreElements()){
         * node=(Node)eenum.nextElement(); gdi = gdiAnn.get(node);
         * if(gdi.rank>max) max=gdi.rank; } System.out.println("ranks: "+max); }
         */
    }

    /**
     * Constructs an initial feasible tree. A tree is feasible if it induces a
     * feasible ranking. A feasible ranking is one satisfying the length
     * constraints e.len>=e.minlen for all edges e (e.len = e.to.rank -
     * e.from.rank).
     */
    protected void feasibleTree() {
        initRank(); // initial feasible ranking
        tightTree(); // spanning tree (only tight edges)
        new LLWalker(graph, graph.vertexSet().iterator().next(), gdiAnn,
    ...
    protected void initRank() {
        /*
         * What is done here: To find the rank of a node, compute the ranks of
         * its in-nodes, add the minimum length of the corresponding edge and
    ...
    protected void tightTree() {
        ...
        while (ttw.getCount() != size) {
            /*
             * 1. find non-edge incident to a tree node with minimal slack 2.
             * adjust ranks of tree nodes
             */
            ...
            ttw = new TightTreeWalker(graph, v, gdiAnn,
                    GraphWalker.InitCode.RESET);
            ttw.run();
        }
    }
    ...
    /**
     * Finds a non-tree-edge incident on the tree with a minimal amount of
     * slack. This slack is the value, the ranks of the treenodes have to be
     * increased or decreased (depending on the direction of the edge), to make
     * this edge tight.
     */
    protected int findDeltaRank() {
        ...
        for (E edge : graph.edgeSet()) {
            ...
            if (gdi1.tree != gdi2.tree) {
                ...
                /*
                 * if(sl==0){ System.out.print(gdi2.rank+" ");
    ...
    protected void initCutValues() {
        /*
         * how: find a node with exact one tree-edge without cutvalue get
         * cutvalue for this edge loop until all tree-edges have a valid
         * cutvalue
         * 
         * meaning of variables: node.mark: number of tree-edges without
         * cutvalue (cut==INT_MIN) next: vector with nodes where node.mark==1
    ...
    protected E getCutValue(V v) {
        ...
        /*
         * 1. E=tree-edge without cutvalue cut=E.weight (following steps take
         * only edges adjacent to v) 2. forall (non-tree-edges e) same direction
         * as E: cut+=e.weight opposite direction : cut-=e.weight 3. forall
         * (tree-edges e with known cutvalue) same direction as E:
         * cut-=(e.cut-e.weight) opposite direction : cut+=(e.cut-e.weight) 4.
         * store cut to E.cut
         */
    ...
    /**
     * Returns a non-tree edge to replace <tt>edge</tt>. All edges going from
     * the head component to the tail component of <tt>edge</tt> are considered,
     * with an edge of minimum slack being chosen to maintain feasibility.
     */
    protected E enterEdge(E edge, ArrayList<E> nonTree) {
        ...
        for (E e : nonTree) {
            ...
            if ((!rev && !ftail && ttail) || (rev && ftail && !ttail)) { // e is
                                                                         // headed
                                                                         // from
                                                                         // head
                                                                         // to
                ...
                if (sl < minSlack) {
                  ...
                  * else if (sl==minSlack) { int nc = gdiFrom.lim > gdiTo.lim ?
                  * gdiFrom.lim : gdiTo.lim; nc -= gdiFrom.low < gdiTo.low ?
                  * gdiFrom.low : gdiTo.low; if (sl < minSlack || nc < cost) {
                  * result = e; //minSlack = sl; cost = nc; } }
                  */
            }
        }
    ...
    /**
     * Returns true if from-component has more nodes. <tt>edge</tt> is a
     * tree-edge. If it is removed the tree is split into two parts - a head and
     * a tail component. Assuming that the low/lim values of the nodes are set
     * properly this method finds out which part of the tree contains more
     * nodes.
     */
    ...
    /**
     * <tt>e2</tt> becomes a tree-edge instead of <tt>e1</tt>. Then the tree and
     * its cutvalues are updated, as well as the vector negCut which contains
     * all tree-edges with negative cutvalue.
     */
    protected void exchange(E e1, E e2, ArrayList<E> negCut,
            ArrayList<E> nonTree) {
        // long t = System.currentTimeMillis();
     ...
     * <ol>
     * <li>cutValues of edges along the path that connects the nodes of the old
     * edge in the new tree
     * <li>low and lim values of all nodes below the least common ancestor of
     * the two nodes of the new edge
     * </ol>
     */
     ...
     * are moved to a fesible rank with the fewest nodes.
     */
    protected void balance() {
        ...
        slack, weightin, weightout; // weights of in-/out-edges
        for (V node : graph.vertexSet()) {
            gdi = gdiAnn.getNode(node);
            ...
            lo = (minSlack < Integer.MAX_VALUE) ? gdi.rank - minSlack
                    : gdi.rank;
            ...
            hi = (minSlack < Integer.MAX_VALUE) ? gdi.rank + minSlack
                    : gdi.rank;
            ...
            if (weightin == weightout && lo < hi) {
                ...
                for (int i = lo + 1; i <= hi; i++)
                    if (rankSize[i] < rankSize[minRank])
                        minRank = i;
    ...
    /**
     * Converts the graph into one whose edges connect only nodes on adjacent
     * ranks. Edges between nodes more than one rank apart are replaced by
     * chains of edges between virtual nodes. These nodes are placed on the
     * intermediate ranks.
     */
    ...
    /*
     * structure: xCoord() buildGraph() getOmega() restoreGraph() yCoord()
     */
    ...
    /**
     * Calculates the x-coordinates for each node of the graph. To do this, the
     * graph is converted into an auxiliary graph. This graph is ranked with the
     * network-simplex algorthm. The ranks of the nodes of this graph are the
     * x-coordinates of the original graph.
     */
     ...
     * Every edge e=(u,v) of the original graph is replaced by a new node (n)
     * and two edges e1=(n,u) and e2=(n,v). These edges have a minlen of 0 and a
     * weight of e.weight*e.Omega.<br>
     * <br>
     * Another class of edges is added to separate nodes on the same rank. If v
     * is the left neighbor of, the the auxiliary graph gets an edge f=(v,w)
     * with a weight of 0 and minlen that results from the width of both nodes
     * and the node-separation value.
     */
    protected HierarchyLayout<V, DefaultEdge> buildGraph() {
        SimpleDirectedGraph<V, DefaultEdge> g = new SimpleDirectedGraph<V, DefaultEdge>(
                DefaultEdge.class);
        Annotation<V, DefaultEdge, GraphDrawInfo<V, DefaultEdge>> xgdiAnn = new Annotation<V, DefaultEdge, GraphDrawInfo<V, DefaultEdge>>(
                g);
        ...
        // ---- Create layouter
        HierarchyLayout<V, DefaultEdge> xlayout = new HierarchyLayout<V, DefaultEdge>(
                g, vertexFactory, null);
    ...
    /**
     * Constructs an initial feasible tree. This method replaces the original
     * feasibleTree() when the ranking-algorithmus for the auxiliary graph is
     * invoked. It uses a more efficient way to construct the initial feasible
     * tree by taking advantage of its structure:
     * <ol>
     * <li>use all edges connecting nodes in the same rank
     * <li>for each pair of adjacent ranks, pick an edge f=(u,v) between the
     * ranks and add both f(u) and f(v) to the tree
     * <li>for every edge e=(w,x)!=f between two ranks, add either e(w) or e(x)
     * to the tree depending on whether w or x is placed leftmost
     * </ol>
     * 
     * @param levels
     *            the ranks to use (obtained from 1st pass, node placement)
     * @param gdiAnn1
     *            the GraphDrawInfo annotation from the 1st pass, to obtain node
     *            positions
     */
    protected void feasibleTree2(Ranks<V, ?> levels) {
        ...
        for (i = levels.cntRanks() - 1; i >= 0; i--) {
            ...
            for (j = 1; j < vec.size(); j++) {
                v1 = vec.get(j);
                gdi1 = gdiAnn.getNode(v1);
                gdi1.rank = gdi2.rank + ((gdi1.width + gdi2.width) >> 1)
                        + nodeSep;
    ...
    public static void main(String[] args) {
        ...
        HierarchyLayout<String, DefaultEdge> layout = new HierarchyLayout<String, DefaultEdge>(
                g, new VertexFactory<String>() {
                    int running = 0;
