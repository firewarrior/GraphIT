...
/**
 * Holds the nodes of a graph in Ranks. The nodes are ordered within its ranks
...
public class Ranks<V, E> {
     ...
     * minimizeCrossings() createVirtualNodes() initOrder() crossing()
     * crossing(int) wMedian() medianValue() sort() transpose()
     * locallayOptimal()
     */
    ...
    /**
     * Initially orders the nodes in each rank. This is done by a BFS starting
     * with vertices of minimum rank. Vertices are placed in their ranks when
     * they are visited by BFS. This ensures that the initial ordering of a tree
     * has no crossings.
     */
    void initOrder() {
        ...
        for (V node : GAlg.topologicalOrder(graph)) {
            ...
            Annotation<V, E, WalkerInfo<V, E>> ann = TypeUtil.uncheckedCast(
                    gdiAnn, null);
    ...
    /**
     * Counts the number of crossings between the given rank and its successor.
     * 
     * @param nr
     *            index of a rank
     */
    protected int crossing(int nr) {
        ...
        for (i = 0; i < edgeCnt - 1; i++) {
            /*
             * for(j=i+1;j<edgeCnt;j++) if(from[i]<from[j] && to[i]>to[j])
             * cnt++;
             */

            for (j = i + 1; j < edgeCnt && from[i] == from[j]; j++)
                /* empty */;
    ...
    /**
     * Reorders the nodes within each rank based on the weighted median
     * heuristic. Depending on <tt>iter</tt> the ranks are traversed from top to
     * bottom (<tt>iter</tt>=even) or from bottom to top (<tt>iter</tt>=odd).
     */
     ...
     * 
     * @param v
     *            the node whose median value is to be calculated
     * @param dir
     *            determines direction (<tt>true</tt> -> forward)
     */
    protected double medianValue(V v, boolean dir) {
        ...
        int i = 0, mid = cnt >> 1;
        int pos[] = new int[cnt];
        for (V node : dir ? GAlg.inNeighboursOf(graph, v) : GAlg
                .outNeighboursOf(graph, v))
            pos[i] = gdiAnn.getNode(node).order;

        Arrays.sort(pos);

        if ((cnt & 1) == 1)
            return pos[mid]; // if cnt is odd

        if (cnt == 2)
            return (pos[0] + pos[1]) / 2.0;

        // cnt is even and >=4
        int left = pos[mid - 1] - pos[0], right = pos[cnt - 1] - pos[mid];
        return ((double) (pos[mid - 1] * right + pos[mid] * left))
                / (left + right);
    }

    /**
     * Sorts the nodes in <tt>rank</tt> by their median value. Nodes with
     * median==-1 (no adjacent nodes) are left in their current positions.
     * 
     * @param nr
     *            index of rank to be sorted
     * @param iter
     *            current iteration
     */
    protected void sort(int nr) {
        ...
        for (i = 0; i < size - 1; i++) {
            ...
            /*
             * It is possible to check medians[j]==medians[min] depending on
             * current iteration (just like in transpose()). But here it seems
             * to be worthless.
             */
            for (j = i + 1; j < size; j++)
    ...
    /**
     * Exchanges the nodes at position <tt>pos1</tt> and <tt>pos2</tt> in rank
     * <tt>nr</tt>. As a side-effect the GrapDrawInfo.order field is updated.
     */
    ...
    /**
     * Exchanges adjacent nodes on the same rank if this decreases the number of
     * crossings.
     */
            ...
            for (i = 0; i < size; i++)
                if (changed[i]) {
                    ...
                    for (j = from; j != to; j += inc) {
                        ...
                        w = ranks.get(i).get(j);
                        inChange = (i == 0) ? 0 : locallyOptimal(v, w, false);
                        outChange = (i == size - 1) ? 0 : locallyOptimal(v, w,
                                true);
     ...
     * number of edge crossings increase when w is placed to the left of v? It
     * is assumed that all out neighbours of v,w are on the rank following v,w.
     * 
     * @param dir
     *            if <tt>true</tt> take forward edges (top to bottom)
     * @return value the number of crossings would change if v and w were
     *         exchanged. A value of -3 means the number of crossings would
     *         decreased by 3.
     */
    ...
    protected int locallyOptimal(V v, V w, boolean dir) {
        ...
        ArrayList<E> wVec = new ArrayList<E>(dir ? graph.outgoingEdgesOf(w)
                : graph.incomingEdgesOf(w));

        for (V vnode : dir ? GAlg.outNeighboursOf(graph, v) : GAlg
                .inNeighboursOf(graph, v)) {
            int pos1 = gdiAnn.getNode(vnode).order;
            for (E e : wVec) {
                V wnode = dir ? graph.getEdgeTarget(e) : graph
                        .getEdgeSource(e);
