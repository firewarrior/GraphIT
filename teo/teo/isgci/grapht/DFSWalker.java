...
public class DFSWalker<V, E> extends GraphWalker<V, E> {
    ...
    /**
     * @param graph
     *            the graph in which to walk
     * @param mark
     *            where to store WalkerInfo
     * @param initCode
     *            determines whether to create a new WalkerInfo, reset it or do
     *            nothing
     */
    public DFSWalker(DirectedGraph<V, E> graph,
            Annotation<V, E, WalkerInfo<V, E>> mark,
            GraphWalker.InitCode initCode) {
        this.graph = graph;
        dynamic = initCode == InitCode.DYNAMIC;
        this.dataAnn = mark != null ? mark : WalkerInfo
                .createAnnotation(graph);
        time = 0;
     ...
     * SimpleDirectedGraph<String,DefaultEdge>(DefaultEdge.class);
     * g.addVertex("q"); g.addVertex("u"); g.addVertex("i"); g.addVertex("c");
     * g.addVertex("k"); g.addEdge("c", "q"); g.addEdge("c", "i");
     * g.addEdge("c", "u"); g.addEdge("c", "k"); g.addEdge("i", "q");
     * g.addEdge("i", "u"); g.addEdge("u", "i"); g.addEdge("q", "u");
     * g.addEdge("i", "c"); g.addEdge("q", "i");
     * 
     * new DFSWalker<String,DefaultEdge>(g, null, GraphWalker.InitCode.DYNAMIC)
     * { public void visit(String v) { System.out.print(v); super.visit(v); }
     * }.discover("q"); System.out.println(); }
     */
}
