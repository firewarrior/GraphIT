...
public class CacheGraph<V, E> extends ListenableDirectedGraph<V, E> implements
    ...
    public void vertexAdded(GraphVertexChangeEvent<V> ev) {
        ...
        if (checking) {
            ...
            if (!graph.containsVertex(nodeCache.get(v)))
                throw new RuntimeException("Error caching addition of node "
                        + v);
        }
    }
    ...
    public void vertexRemoved(GraphVertexChangeEvent<V> ev) {
        ...
        if (checking) {
            ...
            if (graph.containsVertex(v) || containsVertex(v))
                throw new RuntimeException("Error caching removal of node "
                        + v);
        }
    }
    ...
    public void edgeAdded(GraphEdgeChangeEvent<V, E> ev) {
        ...
        if (checking) {
            ...
            if (!graph.containsEdge(findEdge(e)))
                throw new RuntimeException("Error caching addition of edge "
                        + e);
        }
    }
    ...
    public void edgeRemoved(GraphEdgeChangeEvent<V, E> ev) {
        E e = ev.getEdge();
        if (null == edgeCache.remove(new Pair(graph.getEdgeSource(e), graph
                .getEdgeTarget(e))))
            throw new IllegalArgumentException(ev.getEdge()
        ...
        if (checking) {
            ...
            if (graph.containsEdge(e) || containsEdge(e))
                throw new RuntimeException("Error caching removal of edge "
                        + e);
        }
    }
}
