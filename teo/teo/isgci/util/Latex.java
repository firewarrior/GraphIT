...
/**
 * Contains information necessary for displaying latex text. To draw a string,
 * use the following in a derived class: state = new State(s);
...
public abstract class Latex {
    ...
    /**
     * End a subscript by performing the necessary drawing actions and finishing
     * the derived state for it.
     */
    ...
    /**
     * Draws a string (without latex codes) and updates the painting information
     * in state
     * 
     * @param state
     *            the parsing/painting state
     * @param str
     *            the string to draw
     */
        ...
        while (state.i < state.s.length()) {
            ...
            case '\\': // Latex special
                ...
                if (state.s.startsWith("co", state.i)) { // \overline
                    ...
                    endCo(substate);
                    break;
                } else if ((glyph = LatexGlyph.parseGlyph(state.s, state.i)) != null) {
    ...
    protected class State {
        ...
        /**
         * Create a new State with the given one as its parent and with the same
         * settings.
         */
