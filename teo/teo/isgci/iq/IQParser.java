//=========================================================================
//
//  This file was generated by Mouse 1.6 at 2013-07-05 11:01:28 GMT
//  from grammar 'C:\Users\Chris\GitHub\GraphIT\teo\teo\isgci\iq\iq.peg'.
//
//=========================================================================
...
public class IQParser extends mouse.runtime.ParserTest {
    ...
    private boolean expr_0() {
        if (savedInner(expr_0))
            return reuseInner();
        if (!or())
            return rejectInner(expr_0);
        if (!term())
            return rejectInner(expr_0);
    ...
    private boolean term_0() {
        if (savedInner(term_0))
            return reuseInner();
        if (!and())
            return rejectInner(term_0);
        if (!notfactor())
            return rejectInner(term_0);
    ...
    private boolean op() {
        if (saved(op))
            return reuse();
        if (!next("<=", $Term15) && !next('<', $Term7) && !next(">=", $Term16)
                && !next('>', $Term6) && !next('=', $Term3))
            return reject(op);
        space();
        return accept(op);
    }
    ...
    final Cache $Term1 = new Cache("[0-9]");
    ...
    final Cache $Term9 = new Cache("[A-Z]");
    final Cache $Term10 = new Cache("[\r\n]");
    final Cache $Term11 = new Cache("[ \t\r\n]");
    final Cache $Term12 = new Cache("\"_\"");
    final Cache $Term13 = new Cache("_");
    final Cache $Term14 = new Cache("\"or\"");
    final Cache $Term15 = new Cache("\"<=\"");
    final Cache $Term16 = new Cache("\">=\"");
    final Cache $Term17 = new Cache("\")\"");
    final Cache $Term18 = new Cache("\"(\"");
    final Cache $Term19 = new Cache("\"not\"");

    // -------------------------------------------------------------------
    // List of Cache objects
    // -------------------------------------------------------------------

    Cache[] cacheList = { top, expr, term, notfactor, factor, rel, op,
            graphclass, name, thename, id, idstart, idchar, keyword, not, or,
            and, lparen, rparen, space, EOL, EOF, top_0, top_1, expr_0,
            term_0, factor_0, id_0, not_0, $Term0, $Term1, $Term2, $Term3,
            $Term4, $Term5, $Term6, $Term7, $Term8, $Term9, $Term10, $Term11,
