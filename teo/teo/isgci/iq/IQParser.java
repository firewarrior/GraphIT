  ...
  private boolean top_1()
    {
      ...
      do if (!next($Term13)) return rejectInner(top_1);
        while (!EOF());
  ...
  private boolean name()
    {
      ...
      if (!next('"',$Term8)) return reject(name);
      thename();
      if (!next('"',$Term4)
       && !EOL()
         ) return reject(name);
  ...
  private boolean idstart()
    {
      ...
      if (nextIn('A','Z',$Term9)) return accept(idstart);
  ...
  private boolean idchar()
    {
      ...
      if (nextIn('0','9',$Term1)) return accept(idchar);
      if (next('_',$Term12)) return accept(idchar);
  ...
  private boolean not()
    {
      ...
      if (!next("not",$Term19)) return reject(not);
  ...
  private boolean rparen()
    {
      ...
      if (!next(')',$Term17)) return reject(rparen);
      space();
  ...
  private boolean EOF()
    {
      ...
      if (!aheadNot($Term13)) return reject(EOF);
      return accept(EOF);
    }
