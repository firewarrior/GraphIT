...
public class SmallGraphReader extends DefaultHandler {
    ...
    public Collection<SmallGraph> getGraphs() {
        return Collections.unmodifiableSet(new HashSet<SmallGraph>(graphs
                .values()));
    }
    ...
    public void startElement(String uri, String locName, String qName,
        ...
        try {
            ...
            if (qName.equals(SmallGraphTags.SIMPLE)) {
                ...
                int nodeCount = Integer.parseInt(atts
                        .getValue(SmallGraphTags.COUNT));
                ...
                if (current.peekFirst() != null) // Ignore alias in e.g.
                                                 // fakefamily
                ...
                if (current.peekFirst() != null) // Ignore links outside graphs
                    current.peekFirst().graph.addLink(atts
                            .getValue(SmallGraphTags.ADDRESS));
                ...
                if (name.equals(g.getName())) { // Self-complementary
                                                // graph/config?
                    g.setComplement(g);
                ...
                if (current.isEmpty()) {
                    ...
                    if (name == null || "".equals(name))
                        throw new SAXException(
                                "HMT-grammar outside family and"
                                        + " without a name!");
                    ...
                    curgrammar = new HMTGrammar(Integer.parseInt(atts
                            .getValue(SmallGraphTags.TYPE)), name);
                    grammars.put(name, curgrammar);
                } else {
                    curgrammar = new HMTGrammar(Integer.parseInt(atts
                            .getValue(SmallGraphTags.TYPE)));
                    ((HMTFamily) current.peekFirst().graph)
                            .setGrammar(curgrammar);
                }
            }
                ...
                if (current.peekFirst().graph instanceof HMTFamily) {
                    ...
                    HMTGrammar g = grammars.get(atts
                            .getValue(SmallGraphTags.NAME));
                    if (g == null)
                ...
                inclusions.add(new Pair(atts.getValue(SmallGraphTags.SUPER),
    ...
    public void endElement(String uri, String locName, String qName)
        ...
        try {
            if (qName.equals(SmallGraphTags.ROOT_SMALLGRAPHS)) {
                ...
                curgrammar.setHead((Graph) current.peekFirst().graph,
                        attachment);
                attachment = null;
                current.removeFirst();
            }
                ...
                curgrammar.setTail((Graph) current.peekFirst().graph,
                        extension);
                extension = null;
                current.removeFirst();
            }
                ...
                int nodeCount = ((Graph) current.peekFirst().graph)
                        .countNodes();
    ...
    /**
     * Adds the given name to g, if it is not used already, and the name - graph
     * combination to the hashmap. Returns true iff succesful.
     */
        ...
        public void complete() {
            ...
            for (ArrayList<String> ss : induced) {
                ...
                for (String s : ss) {
                    ...
                    SmallGraph g = graphs.get(s);
                    if (g == null)
                        System.err.println("Can't find induced " + s + " for "
                                + graph.getName());
                    v.add(g);
                }
                graph.addInduced(v);
            ...
            for (String s : smallmembers) {
                SmallGraph g = graphs.get(s);
                if (g == null)
                    System.err.println("Can't find smallmember " + s + " for "
                            + graph.getName());
...
/**
 * Parses edges and returns them one by one. Used format is (int - int;)*
 */
