...
public class SmallGraphWriter {
    ...
    public void writeSmallGraphs(String xmldecl, Vector smallGraphs,
            ...
            DirectedGraph<Graph, DefaultEdge> incls) throws SAXException {
        if (xmldecl == null)
            ...
            if (f instanceof SimpleFamily) {
                ...
                if (fhmt.getGrammar() != null)
                    if (fhmt.getGrammar().getName() != null) {
                        atts.addAttribute(SmallGraphTags.NAME, fhmt
                                .getGrammar().getName());
    ...
    protected void writeInduced(Vector<Vector<SmallGraph>> list)
        ...
        for (Vector<SmallGraph> vecInd : list) {
            writer.characters("   ");
            if (vecInd.size() == 1) {
                writer.dataElement(SmallGraphTags.INDUCED1, vecInd
                        .firstElement().getName());
            } else {
                writer.startElement(SmallGraphTags.INDUCED);
                for (SmallGraph g : vecInd)
                    writer.dataElement(SmallGraphTags.SMALLGRAPH, g.getName());
    ...
    protected void writeInducedRest(Vector<Vector<SmallGraph>> list)
        ...
        for (Vector<SmallGraph> vecInd : list) {
            writer.characters("   ");
            if (vecInd.size() == 1) {
                writer.dataElement(SmallGraphTags.INDUCEDREST1, vecInd
                        .firstElement().getName());
            } else {
    ...
    protected void writeGrammar(HMTGrammar gram, String indent)
        ...
        writer.characters(indent);
        atts.addAttribute(SmallGraphTags.TYPE,
                (new Integer(gram.getType())).toString());
    ...
    protected void writeEdges(DirectedGraph<Graph, DefaultEdge> incls)
        ...
        for (DefaultEdge e : incls.edgeSet()) {
            atts.addAttribute(SmallGraphTags.SUPER, incls.getEdgeSource(e)
                    .getName());
            atts.addAttribute(SmallGraphTags.SUB, incls.getEdgeTarget(e)
                    .getName());
